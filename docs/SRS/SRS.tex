...
This is the Software Requirements Specification of FastAid. FastAid is an online, real-time responders for disaster relief and rehabilitation.Emergency response platform that connects emergency victims with trained first responders to relief and rehabilitation in events. aid volunteer responders. The idea behind FastAid was inspired by the many delays that occur. assisted by the high dispatch delay and the high volume of traffic of the conventional emergency services.Supported by the high volume of traffic and dispatch delay of the conventional emergency services. urban environments. Many times, critical life-saving interventions (CPR, hemorrhage control, etc.) In the case of a failure of control (control), victims are not reached in time.

FastAid is here to make that happen. The mobile device triggers an emergency request by calling a number, victim automatically broadcasts their GPS location. The verified data is matched with the FastAid instantly. volunteerswithina5kmradius. ThesystemalsoleveragesanAIenginetoanalyzetheemergency Describe and give an immediate severity grading and advice to the responder. 

The overarching goals are simple:
\begin{itemize}[leftmargin=2em]
  \item Provide a super-fast way for victims to send immediate help to a medical facility, automatically using GPS to locate the nearest one. capture.
  \item Use WebSockets to dynamically track volunteer locations to ensure that the closest volunteers are available. are emphasised and noticeable for the victim.
  \item LeverageAIanalysistoevaluatetheemergencydescriptionandmentallypreparevolunteers while en route.
  \item Ensure a secure administrative area and check medical credentials prior to volunteering teers go live.
\end{itemize}
...
FastAid is designed for addressing the needs of urban populations and where ambulance response time is often more than 10-15 minutes. minutes. The system aims to fill this gap by soliciting a crowdsourced intervention of the medical nature, which is verified. Our Research suggests that prompt first aid can improve survival rates in life-threatening scenarios.

\subsection{Purpose}
Explain the operation of FastAid, its development and, its system constraints. This encompasses functional requirements (what the system does), non The functional requirements (system effectiveness), and architectural requirements. 

\subsection{Documents Conventions}
High-level terms, headings and features are highlighted in bold.
\begin{itemize}[leftmargin=2em]
  \item The terms used in the survey data and explanations are in italics.
  \item Codeisusedtoshowtherootpath,databasefieldnames,andtechnicalIDs (e.g. \texttt{/admin/users}, \texttt{lastKnownLocation}).
  \item This is done to indicate the text in the labels and buttons (such as Start GPS sharing,” “Accept”). Case”).
\end{itemize}

\subsection{For whom is the book meant and how should it be read?}
The target audience of this document is:
\begin{enumerate}[leftmargin=2em]
  \item Developers – as a strong reference throughout development.
  \item Project Managers- to know feature priorities and implementation logic.
  \item System Administrators- who oversee operations and need to understand the RBAC (Role Based Access Control) mechanisms are the mechanisms that are based on the user\'s identity.
\end{enumerate}

\subsection{Problem Definition}
In urban areas, citizens are heavily affected by delays in ambulance dispatch. Re According to search, every minute counts when it comes to medical emergencies and the number of seconds that pass significantly reduces chances of survival. the survival rate. The situation is exacerbated by:
\begin{enumerate}[leftmargin=2em]
  \item \textbf{Traffic Congestion}: Ambulances are unable to go round the traffic snarl ups during the peak hours in the city.
  \item \textbf{Lack of Immediate Care}: Bystanders have not the confidence or training to react to an emergency. intervene.
  \item \textbf{Information Asymmetry}: Even with assistance, they may not have context knowledge. Independent of the victim\'s previous state of health or the actual extent of the trauma.
\end{enumerate}
By utilizing the hyper-local network of volunteers in the community, FastAid is able to address these issues. On foot or light vehicles, with Artificial Intelligence analysis of the crisis.
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item AI Integration: Automated severity assessment, key injuries and medical advice provided through an a LLM service.
  \item Real-Time Tracking: Socket.Radio broadcasting for the location of volunteers with streams from IO.
...
\textbf{Demographics:} 5 respondents, off duty paramedics, nurses, and former victims of traffic accidents. 
...
\textbf{Key Insight —“Preparedness Gap”:} clearly Thedata indicates there is an interest in being volunteers, but not prepared. To help, they will not plunge into an unfamiliar scenario. FastAid should be a smart person. Dispatcher, sum up a victim\'s panic-typed messages in actionable, using AI, Medical reason if this is for the responder.

\subsection{Document Analysis Results}
There is overwhelming research evidence to support the application of crowdsourced response. Studies show that A strict 5km geolocation matching with atomic transaction assign is used in systems. Mentions prevent from double booking of the same emergency and from the ”bystander effect”.
...
The following critical features were incorporated into the FastAid based on the elicitation responses: platform:
...
F1 & Auto-GPS Request Capture & Victimsunderextremestresscannottypeanaddressaccurately. Using one button to access HTML5 Geolocation is mandatory. \\
\hline
F2 & AI Emergency Analysis & The volunteers ought to know whether the victim is bleeding or not, unconscious, or burned. Translating frantic descrip with the LLM. A break down of medical severity codes and advice. \\
\hline
F3 & Web-Push with SMS Fallback & When there are urban dead-zones, data packets drop. If a Web-Push fails The system should send a dependable SMS after 2000ms. A fall back mechanism should be in place to ensure that the volunteer is notified. \\
\hline
F4 & Live Socket.IO Tracking & Victims are very anxious about being rescued. Watchingthevolunteer’sGPSdotmoveontheLeaflet mapinreal-timeprovidesimmensepsychologicalrelief. \\
\hline
F5 & Medical Profiles & Respondersneedtoknowif thevictimhaspre-existing conditions(e.g.,Asthma,Hemophilia)orspecificblood typesbeforeadministeringcare. \\
\hline
\end{longtable}
...
An overview of the interactions between the three primary roles and the system is given in Figure~\ref{fig:usecase}.
...
Figure~\ref{fig:sequence} depicts the process of an emergency request being analyzed, dispatched, and accepted by a volunteer.
...
Figure~\ref{fig:activity} illustrates the complete lifecycle of an emergency from creation to closure, including the AI analysis branch and the notification fallback mechanism.
...
\begin{enumerate}[leftmargin=2em]
  \item A victim wishes to register by means of mobile/email and have their medical profile (allergies, blood type) safely kept so that the responders will be informed in case of a crisis.
  \item A volunteer wishes to upload their medical qualifications and switch on or off their availability status when they are off duty.
\end{enumerate}
...
{The user can put in the \texttt{medicalProfile} embedded document: \texttt{bloodType}, \texttt{allergies}, \texttt{preExistingConditions}, \texttt{emergencyContactName}, and \texttt{emergencyContactPhone}.}
{%
\begin{itemize}[leftmargin=1.5em]
  \item Updates are immediately reflected in all subsequent emergency responses.
  \item The emergency contact fields allow responders to contact a family member.
\end{itemize}
}
...
\subsubsection*{User Stories}
\begin{enumerate}[leftmargin=2em]
  \item A victim wishes to press a single button and have their GPS automatically taken so that they do not squander their time in typing addresses in a crisis.
  \item The victim wants the system to understand the nature of the crisis and provide severity information to volunteers approaching the scene.
...
\subsubsection*{User Stories}
\begin{enumerate}[leftmargin=2em]
  \item A victim wants to observe the volunteer\'s position tracking on a live map to ensure that they are getting help.
  \item A volunteer would like the system to match them to the nearest emergency immediately so that they travel less.
\end{enumerate}
...
\subsubsection*{User Stories}
\begin{enumerate}[leftmargin=2em]
  \item An administrator wants a summary of system status in general, containing uptime, active users, and pending certificates.
  \item An admin must confirm volunteer medical credentials before dispatching.
  \item An administrator wishes to view audit logs of every emergency state transition.
\end{enumerate}
...
FastAid Software Requirements Specification solidifies the operational blue prints of a fast, crowdsourced medical dispatch system. Describing in detail the 16 func tional requirements for 6 components---from Auto-GPS capture and AI severity analysis to atomic transactional case acceptance and live WebSocket tracking---this SRS ensures that FastAid can safely and securely meet the demands of urban emergencies.

The key differentiators from the original SRS are the deletion of the Rat ing/Feedback systems (which created friction with no value) and the inclusion of AI Emergency Analysis, real-time Socket.IO GPS streaming and a rigorous 5-state volunteer verification process. These changes are based on the survey data (the “Preparedness Gap”) as well as the literature on crowdsourced emer gency response \cite{zijlko2021, hasselqvist2015}.

This document specifies one critical requirement that is partially imple mented—the mock authentication system (\texttt{x-demo-role})—which is supposed to be re placed with a JWT or session-based authentication before deployment. All other prerequisites have been fulfilled and can be traced back to the current base code.
